...
class Upsample(nn.Module):
    def __init__(self, num_channels):#, num_joints, num_frames, kernel_size
        ...
        #self.conv = nn.Conv2d(num_channels*2, num_channels, kernel_size=1)
    def forward(self, x):
        ...
        ##head = self.up_head(x[:, :, :, [0]])
        #torso = self.up_torso(x[:, :, :, [1]])
        #left_arm = self.up_left_arm(x[:, :, :, [2]])
        #right_arm = self.up_right_arm(x[:, :, :, [3]])
        left_leg = self.up_left_leg(x[:, :, :, [4]])
        right_leg = self.up_right_leg(x[:, :, :, [5]])
