...
class GroupToPoint(nn.Module):
    def __init__(self, groups, in_channels):
        ...
        self.weights = nn.ParameterDict({
            group_name: nn.Parameter(torch.ones(len(indices), in_channels))
    ...
    def forward(self, x):
        ...
        for group_name, indices in self.groups.items():
            group_feature = x[:, :, :, indices]  # (N, C, T, len(indices))
            weights = self.weights[group_name].unsqueeze(0).unsqueeze(2)  # (1, len(indices), 1)
            weighted_feature = (group_feature * weights).sum(dim=-1, keepdim=True)  # (N, C, T, 1)
            grouped_points.append(weighted_feature)
        return torch.cat(grouped_points, dim=-1)  # (N, C, T, num_groups)
"""
...
class Model(nn.Module):
    def __init__(self, num_classes, num_joints, num_frames, num_persons, 
                 num_heads, num_channels, kernel_size, use_pess=True,
                 use_pest=True, spatial_att=True, temoral_att=True, 
    ...
    def forward(self, x):
        ...
        #Encoder Blocks
        x = self.en0_0(x) #(64, 64, 120, 25)
        x = self.en1_0(x) #(64, 64, 120, 25)
        x = self.en2_0(x) #(64, 128, 120, 25)
        x = self.en3_0(x) #(128, 128, 120, 25)
        x = self.en4_0(x) #(128, 256, 120, 25)
        x = self.en5_0(x) #(256, 256, 120, 25)
        x = self.en6_0(x) #(256, 256, 120, 25)
        x = self.en7_0(x) #(256, 256, 120, 25)
