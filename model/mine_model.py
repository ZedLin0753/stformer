...
class Model(nn.Module):
    def __init__(self, num_classes, num_joints, 
        ...
        #Encoder Blocks
        for index, (in_channels, out_channels, qkv_dim) in enumerate(self.encoder_config):
        ...
        #Bottleneck Blocks
        #for index, (in_channels, out_channels, qkv_dim) in enumerate(self.bottleneck_config):
            #print('in_channel: ', in_channels)
            #print('out_channel: ', out_channels)
            #print('qkv_dim: ', qkv_dim)
            #print('########################')
        self.bottleneck.append(TRANS_Block(in_channels=bottleneck_config[0], out_channels=bottleneck_config[1], qkv_dim=bottleneck_config[2], 
                                         num_frames=num_frames, 
                                         num_joints=num_joints, 
                                         num_heads=num_heads,
                                         kernel_size=kernel_size,
    ...
    def forward(self, x):
        ...
        #Position Embedding
        x1 = self.pes(x) + x if self.use_pes else x
        ...
        for iex, block in enumerate(self.encoders):
            x = block(x)
            ...
            #print('len(encoder_features): ', len(encoder_features))
        #print('x.type: ', x.type())

        #BottleNeck
        for ibx, block in enumerate(self.bottleneck):
            x = block(x)
            #print('x.shape: ', x.shape)
