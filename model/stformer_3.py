...
class Model(nn.Module):
    def __init__(self, num_classes, num_joints, num_frames, num_persons, 
        ...
        # 分群模組
        self.group_to_point64 = GroupToPoint(GROUPS, 64)
        self.group_to_point128 = GroupToPoint(GROUPS, 128)
        self.group_to_point256 = GroupToPoint(GROUPS, 256)
    ...
    def forward(self, x):
        ...
        # 原始關節特徵
        y = self.input_map(x)  # (N*M, C, T, V)
        #print('y.shape: ', y.shape)

        # 分群後特徵
        z = self.group_to_point64(y)  # (N*M, C, T, 6)
        #print('z.shape: ', z.shape)

        # 經STA編碼器
        y = self.en0_0(y)  #(N*M, 64, 120, 25)
        z = self.en0_1(z)  #(N*M, 64, 120, 6)
        g = self.c_att0_0(y, z) #(N*M, 64, 120, 25)

        g1 = self.group_to_point64(g)  # (N*M, C, T, 6)

        y = self.en1_0(g)  #(N*M, 64, 120, 25)
        z = self.en1_1(g1)  #(N*M, 64, 120, 6)
        y = self.en2_0(y)  #(N*M, 128, 120, 25)
        z = self.en2_1(z)  #(N*M, 128, 120, 6)
        g = self.c_att1_0(y, z) #(N*M, 128, 120, 25)

        g2 = self.group_to_point128(g)  # (N*M, C, T, 6)

        y = self.en3_0(g)  #(N*M, 128, 120, 25)
        z = self.en3_1(g2)  #(N*M, 128, 120, 6)
        y = self.en4_0(y)  #(N*M, 256, 120, 25)
        z = self.en4_1(z)  #(N*M, 256, 120, 6)
        g = self.c_att2_0(y, z) #(N*M, 256, 120, 25)

        g3 = self.group_to_point256(g)  # (N*M, C, T, 6)

        y = self.en5_0(g)  #(N*M, 256, 120, 25)
        z = self.en5_1(g3)  #(N*M, 256, 120, 6)
        y = self.en6_0(y)  #(N*M, 256, 120, 25)
        z = self.en6_1(z)  #(N*M, 256, 120, 6)
        y = self.en7_0(y)  #(N*M, 256, 120, 25)
        z = self.en7_1(z)  #(N*M, 256, 120, 6)
        g4 = self.c_att2_0(y, z) #(N*M, 256, 120, 25)


        # 全局池化與分類
        final = g4.view(N, M, self.out_channels, -1)
